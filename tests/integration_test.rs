...
fn test_merge_with_sorting() {
    ...
    assert_eq!(
        lines, expected_records,
        "Records are not in the expected order"
    );
...
fn test_split_file() {
    ...
    assert!(
        !entries.is_empty(),
        "No output CSV files found in directory"
    );

    // Collect all records from all output files
    let mut all_records = Vec::new();
    for entry in &entries {
        let content = fs::read_to_string(entry.path()).expect("Failed to read output file");
